...
| D. Protected numeric mapping + repository-scoped HMAC | 最小公開資料、deterministic、fail closed | 需 secret lifecycle 與 Environment bootstrap | Proposed |

## ADR

- `ADR-001`：Protected numeric role mapping and repository-scoped pseudonymous GitHub identity（Proposed；待 G3 決定）。
